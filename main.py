...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        c.executemany("INSERT INTO products (name, description, price, category, image_url, stock) VALUES (?, ?, ?, ?, ?, ?)", sample_products)
        conn.commit()
    conn.close()
...
def create_product(product: Product):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO products (name, description, price, category, image_url, stock) VALUES (?, ?, ?, ?, ?, ?)",
